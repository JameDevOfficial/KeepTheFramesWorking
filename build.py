...
def create_windows_exe(love_file, love_version, arch, output_dir):
    ...
    game_exe = love_dir / "KeepTheFramesWorking.exe"
    
    print(f"Creating KeepTheFramesWorking.exe ({arch}-bit)...")
    ...
    # Create zip archive
    output_zip = output_dir / f"KeepTheFramesWorking-win{arch}.zip"
...
def main():
    ...
    web = releases / "web"
    love_file = releases / "KeepTheFramesWorking.love"
    ...
    if is_wsl:
        ...
        lovejs_cmd = f"npx love.js.cmd -c \"{love_file_win}\" \"{web_win}\" --title \"KeepTheFramesWorking\""
    else:
        lovejs_cmd = f"npx love.js.cmd -c \"{love_file}\" \"{web}\" --title \"KeepTheFramesWorking\""
    ...
    checks = {
        ".love file": love_file,
        "Windows 32-bit": executables / "KeepTheFramesWorking-win32.zip",
        "Windows 64-bit": executables / "KeepTheFramesWorking-win64.zip", 
